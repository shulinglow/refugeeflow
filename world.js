...
d3.csv("population_new.csv").then(data => {
    ...
    .attr("paint-order", "stroke fill");
            ...
            const colorScale = d3.scaleSequential()
                ...
                .on("mouseover", (event, d) => {
                    tooltip.transition()
                        .duration(200)
                        .style("opacity", .9);
                    tooltip.html(`Year: ${year}<br>Total Refugee Count: ${totalRefugeesByYear}`)
                        .style("left", (event.pageX + 10) + "px")
                        .style("top", (event.pageY - 28) + "px");
                })
                .on("mouseout", (event, d) => {
        ...
        function getRefugeeCountForCountry(countryName, data, year) {
            const countryData = data.filter(d => d.Asylum === countryName && d.year === year);
            let totalRefugees = 0;
